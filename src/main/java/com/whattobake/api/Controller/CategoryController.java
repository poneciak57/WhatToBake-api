...
public class CategoryController {
    ...
    public Flux<CategoryDto> allCategories() {
        return categoryService.allCategories().map(CategoryDto::fromCategory);
    }

    @GetMapping("/{id}")
    public Mono<CategoryDto> oneById(@Min (0) @NotNull @PathVariable("id") Long id) {
        return categoryService.getOneById(id).map(CategoryDto::fromCategory);
    }

    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("")
    public Mono<CategoryDto> newCategory(@Valid @RequestBody Mono<CategoryInsertRequest> categoryInsertRequest) {
        return categoryInsertRequest.flatMap(categoryService::newCategory).map(CategoryDto::fromCategory);
    }
    ...
    public Mono<CategoryDto> updateCategory(
        ...
        return categoryInsertRequest.map(c -> c.toUpdateRequest(id))
                .flatMap(categoryService::updateCategory).map(CategoryDto::fromCategory);
    }
